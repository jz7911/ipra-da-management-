...
function Modal({ children, onClose, wide }) {
  return (
    ...
    >
      <div style={{ background: '#fff', border: '0.5px solid rgba(0,0,0,0.18)', borderRadius: 12, padding: 26, width: wide ? 600 : 480, maxWidth: '96vw', marginBottom: 40 }}>
        {children}
      </div>
    </div>
  )
...
<style>
body{font-family:Georgia,serif;max-width:820px;margin:30px auto;padding:0 24px;color:#1a1a1a;line-height:1.75;font-size:13px}
h1{font-size:22px;font-weight:600;border-bottom:2px solid #1D9E75;padding-bottom:10px;margin-bottom:24px}
h2{font-size:17px;font-weight:600;margin-top:42px;padding:7px 14px;background:#f5f5f3;border-left:4px solid #888}
h3{font-size:14px;font-weight:600;margin-top:22px;color:#444}
.std{border:1px solid #ddd;border-radius:5px;padding:14px;margin:12px 0;page-break-inside:avoid}
.badge{font-size:10px;padding:2px 6px;border-radius:3px;font-weight:700;margin-left:6px}
.M{background:#fee2e2;color:#991b1b}.O{background:#dcfce7;color:#166534}
.commentary{font-size:12px;color:#555;font-style:italic;margin:6px 0 8px;line-height:1.6}
.ev-label{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#888;margin-bottom:5px}
.ev{font-size:12px;padding:2px 0 2px 14px;position:relative;line-height:1.5}
.ev:before{content:"•";position:absolute;left:3px;color:#888}
@media print{h2{page-break-before:always}h2:first-of-type{page-break-before:avoid}.std{page-break-inside:avoid}button{display:none}}
    ...
    selectedCats.forEach(cat => {
      ...
      cat.sections.forEach(sec => {
        ...
        sec.standards.forEach(s => {
          html += `<div class="std">
<strong>${s.std_key} ${s.title}</strong><span class="badge ${s.type}">${s.type === 'M' ? 'MANDATORY' : 'OPTIONAL'}</span>
${s.commentary ? `<div class="commentary"><strong style="font-style:normal">Commentary:</strong> ${s.commentary}</div>` : ''}
<div class="ev-label">Evidence of compliance</div>
  ...
  return (
    <Modal onClose={onClose} wide>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 22 }}>
        {[
          { key: 'word', icon: '📄', title: 'Word document', desc: 'Downloads a .docx file. Opens in Word, Google Docs, or LibreOffice.' },
          { key: 'pdf', icon: '🖨️', title: 'PDF', desc: 'Opens a print-ready page. Use Print → Save as PDF in your browser.' }
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
        ...
        <button
          ...
          onClick={() => format === 'word' ? exportWord() : exportPDF()}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#f2f1ef' }}>
      ...
      <div style={{ height: 56, background: '#fff', borderBottom: '0.5px solid rgba(0,0,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {activeYear && (
            <span style={{ fontSize: 12, fontWeight: 500, padding: '4px 10px', background: '#e6f1fb', color: '#0c447c', border: '0.5px solid #B5D4F4', borderRadius: 20 }}>
              {activeYear.year}
            </span>
          )}
          {profile?.full_name && (
          ...
          <button
            ...
            style={{ background: '#e1f5ee', color: '#085041', border: '0.5px solid #9FE1CB', fontWeight: 500, fontSize: 12, padding: '5px 14px', borderRadius: 8, cursor: 'pointer', fontFamily: 'inherit' }}>
            Export
          </button>
          <button
            onClick={() => supabase.auth.signOut()}
            style={{ fontSize: 12, color: '#999992', padding: '5px 12px', background: 'none', border: '0.5px solid rgba(0,0,0,0.12)', borderRadius: 8, cursor: 'pointer', fontFamily: 'inherit' }}>
            Sign out
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{ width: 215, flexShrink: 0, background: '#fff', borderRight: '0.5px solid rgba(0,0,0,0.1)', overflowY: 'auto' }}>
          ...
          {cats.map(cat => (
            <div key={cat.key} onClick={() => { setCatKey(cat.key); setSecKey(null) }}
              style={{
                display: 'flex', alignItems: 'center', gap: 9, padding: '8px 14px', cursor: 'pointer', fontSize: 13,
                color: catKey === cat.key ? '#1a1a1a' : '#555550',
                fontWeight: catKey === cat.key ? 500 : 400,
                background: catKey === cat.key ? '#f8f8f7' : 'transparent',
                borderLeft: `3px solid ${catKey === cat.key ? cat.dot_color : 'transparent'}`
              }}>
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: cat.dot_color, flexShrink: 0 }} />
              {cat.label}
            </div>
        ...
        <div style={{ width: 235, flexShrink: 0, background: '#f8f8f7', borderRight: '0.5px solid rgba(0,0,0,0.1)', overflowY: 'auto' }}>
          {!catKey
            ? <div style={{ padding: 20, fontSize: 13, color: '#999992' }}>Select a category</div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ padding: '8px 20px', borderBottom: '0.5px solid rgba(0,0,0,0.1)', background: '#fff', flexShrink: 0 }}>
            <input
              ...
              placeholder="Search by ID, title, commentary, or evidence…"
          ...
          <div style={{ flex: 1, overflowY: 'auto', background: '#fff' }}>
            {search.trim().length > 1 ? (
              <div style={{ padding: '0 24px 24px' }}>
                <div style={{ padding: '18px 0 14px', fontSize: 18, fontWeight: 600, color: '#1a1a1a' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#999992', fontSize: 14 }}>
              ...
              <div style={{ padding: '0 24px 24px' }}>
                ...
                {currentSec?.standards.length === 0 && (
                  <div style={{ textAlign: 'center', padding: '50px 0', color: '#999992', fontSize: 14 }}>No standards in this section.</div>
                )}
                {currentSec?.standards.map(std => <StdCard key={std.id} std={std} />)}
              </div>
            )}
          </div>
        </div>
      ...
      {showExport && <ExportModal onClose={() => setShowExport(false)} cats={cats} year={activeYear?.year} />}
